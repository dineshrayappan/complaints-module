...
// Update status to In Progress (Action Person, Auditor, or Admin)
router.patch('/:id/in-progress', verifyToken, markInProgress);

// Submit Action Resolution (ACTION_PERSON role strictly enforced + Multer file upload for After Photo)
router.post(
  '/:id/submit-action',
  verifyToken,
  requireRole(['ACTION_PERSON']),
